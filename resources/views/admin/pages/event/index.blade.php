...
<div class="space-y-8">
    ...
    @if(session('success'))
    <div class="rounded-2xl border border-emerald-100 bg-emerald-50 px-5 py-3 text-sm font-medium text-emerald-700">
        {{ session('success') }}
    </div>
    @endif

    @if(session('error'))
    <div class="rounded-2xl border border-rose-100 bg-rose-50 px-5 py-3 text-sm font-medium text-rose-700">
        {{ session('error') }}
    </div>
    @endif
    ...
    @php
        $events = $events ?? [];
        $totalEvents = $totalEvents ?? (method_exists($events, 'total') ? $events->total() : count($events));
        $activeEvents = $activeEvents ?? collect($events)->where('status', 'published')->count();
        $inactiveEvents = $inactiveEvents ?? collect($events)->where('status', '!=', 'published')->count();
        $eventPerPageOptions = $eventPerPageOptions ?? [10, 25, 50, 100];
    @endphp
    ...
    <div class="bg-white border border-gray-100 rounded-3xl shadow-[0_4px_20px_-4px_rgba(6,81,237,0.05)] overflow-hidden">
                ...
                <tbody class="divide-y divide-gray-100 bg-white">
                    ...
                    <tr class="hover:bg-indigo-50/30 transition-colors group">
                        ...
                        <td class="px-6 py-5 whitespace-nowrap">
                            <div class="flex items-center gap-2">
                                ...
                                <span class="text-sm text-gray-700">
                                    {{ !empty($event['event_date']) ? \Carbon\Carbon::parse($event['event_date'])->format('d M Y') : '-' }}
                                    @if(!empty($event['start_time']))
                                        <span class="text-gray-400">•</span> {{ substr($event['start_time'], 0, 5) }}
                                    @endif
                                </span>
                            </div>
                        </td>
                        ...
                        <td class="px-6 py-5 whitespace-nowrap">
                            ...
                            @elseif(($event['status'] ?? 'draft') === 'closed')
                            <span class="inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-semibold bg-slate-100 text-slate-700 border border-slate-200 shadow-sm">
        ...
        @include('company.components.table-pagination', [
            'id' => 'events',
            'paginator' => $events,
            'rowsPerPageOptions' => $eventPerPageOptions,
            'label' => 'Events table pagination',
        ])
        @endif
    </div>
